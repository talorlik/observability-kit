...
| 27 | `Do batch 27: demo workloads and observability playground.` | Validate demo architecture ADR and package skeleton with onboarding-contract conformance, multi-kind sample service renders with signal inventory, traffic scenario schema checks with seeded-invalid rejection, demo dashboard provisioning-path checks with standard filters, MCP-bound AI prompt pack coverage, and playground guide completeness with product-docs registration. |
